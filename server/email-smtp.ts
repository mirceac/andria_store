...
function createMailgunClient() {
  console.log("Creating Mailgun client with configuration:");
  
  const apiKey = process.env.MAILGUN_API_KEY || '';
  const domain = process.env.MAILGUN_DOMAIN || '';
  const fromEmail = process.env.MAILGUN_FROM_EMAIL || `Andria Store <postmaster@${domain}>`;
  
  console.log("Mailgun Configuration:");
  console.log("Domain:", domain);
  console.log("From Email:", fromEmail);
  console.log("API Key configured:", !!apiKey, apiKey ? `(length: ${apiKey.length})` : '');
  
  // Check for missing required configuration
  const missingConfig = [];
  if (!apiKey) missingConfig.push('MAILGUN_API_KEY');
  if (!domain) missingConfig.push('MAILGUN_DOMAIN');
  ...
  console.log("✓ Mailgun client created");
  return { mg, domain, fromEmail };
}
...
export async function sendOrderConfirmationEmail(
  ...
  try {
    ...
    const { mg, domain, fromEmail } = createMailgunClient();
